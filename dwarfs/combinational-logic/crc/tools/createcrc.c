...
int main(int argc, char** argv)
{
	...
	if(do_save)
	{
		if(!file_path)
		{
			...
			t = time(NULL);
			tm = *localtime(&t);
			snprintf(file_path,(sizeof(char)*CRC_NAME_MAX_LENGTH),"../data/crcfile_N%u_S%u_%d-%d-%d-%d-%d",num_pages,page_size,(tm.tm_year-100),tm.tm_mon+1,tm.tm_mday,tm.tm_hour,tm.tm_min);
			free_file=1;
		}
