...
void BFSGraph(int argc, char ** argv)
{
    ...
    fprintf(stderr, "Initializing graph...\n");
    InitializeGraph(
        &h_graph_nodes,
        &h_graph_mask,
    ...
    fprintf(stderr, "Starting traversal...\n");
    ...
    fprintf(stderr, "Copied Everything to device memory\n");
    ...
    if(err != CL_SUCCESS)
        fprintf(stderr, "ERROR COMPILING KERNEL 1 (%d)\n", err);
    ...
    if(err != CL_SUCCESS)
    {
        fprintf(stderr, "Error building kernel 1(%d).\n", err);
    ...
    if(err != CL_SUCCESS)
        fprintf(stderr, "Error creating Kernel 1(%d).\n", err);


    cl_kernel kernel2 = clCreateKernel(kernel1Program, "kernel2", &err);
    if(err != CL_SUCCESS)
        fprintf(stderr, "Error creating kernel 2.\n");
    ...
    fprintf(stderr, "maxThreads[0]=%d WorkSize[0]=%d localWorkSize[0]=%d\n", maxThreads[0], WorkSize[0], localWorkSize[0]);
    cl_event syncEvent;
    do
    {
        stop = 0;
        ...
        clFinish(commands);
        if(err != CL_SUCCESS)
            fprintf(stderr, "Error occurred running kernel1.(%d)\n", err);
        err = clEnqueueNDRangeKernel(commands, kernel2, 1, NULL,
                                     WorkSize, localWorkSize, 0, NULL, &ocdTempEvent);
        clFinish(commands);
        if(err != CL_SUCCESS)
            fprintf(stderr, "Error occurred running kernel2.\n");
    ...
    fprintf(stderr, "Total time taken is %.3lf seconds\n", get_interval_by_sec(&sw));
    fprintf(stderr, "First cost is %d\n", h_cost[0]);

    fprintf(stdout, "{\"options\": \"%d\", \"time\": %f, \"status\": %d}\n", no_of_nodes, get_interval_by_sec(&sw), 1);
