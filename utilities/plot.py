...
LANGUAGES = {"js":"js", "C":"c", "asmjs":"asmjs", "OpenCL":"opencl", "WebCL":"webcl", "js-nota":"js-nota"}
...
def itercategories(cs):
    ...
    return [tuple(v for _, v in sorted(x.items())) for x in values if
        ...
        (not ((x["browser"] != "native" and x["language"] in ['c', 'opencl']) or
              (x["browser"] == "native" and x["language"] not in ['c', 'opencl'])))]
